...
public class BinarySearchTree<T extends Comparable<T>> implements BinaryTreeInterface<T> {
        ...
        //elem is in the tree, check whether it is a leaf or an internal node
            ...
            if (toRemove.left == null && toRemove.right == null) {
                ...
                if (toRemove.left == null && toRemove.right != null) {
                    ...
                    //assign the left sub-tree of the maximum to its parent's left child 
                    if(pnode.element.compareTo(node.element) == 1)
    ...
    private void printInternalNodes(TNode<T> current){
        if(current == null)
            return;
        if(current.left != null || current.right != null){
            System.out.print(current.element + " ");
            printInternalNodes(current.left);
            printInternalNodes(current.right);
        }
    }    
}
